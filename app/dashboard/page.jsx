      ...
      <aside className="rp-sidebar hidden md:flex flex-col sticky top-0 h-screen border-r border-[#263143]">
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          ...
          <button
            ...
            className={`w-full flex items-center gap-2.5 px-4 py-2.5 rounded-lg font-bold text-xs transition-all border ${

              activeTab === 'subscription'

                ? 'bg-[rgba(51,149,255,0.15)] text-white border-blue-500/20 shadow-sm'

                : 'bg-white/5 border-white/5 text-slate-350 hover:bg-white/10 hover:text-white shadow-sm'

            }`}

          >

            <Crown className={`w-3.5 h-3.5 shrink-0 ${activeTab === 'subscription' ? 'text-blue-400' : 'text-slate-455'}`} />
