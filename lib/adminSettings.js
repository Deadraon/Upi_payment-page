...
export async function checkAndProcessSubscription(order, apiKey) {
  try {
    const isPlatformMerchant = apiKey === CONFIG.platformApiKey || order.merchant_id === '677d9312-a53f-4b96-815f-53e0eee1b292';
    
    if (isPlatformMerchant && order.external_ref && (order.note?.startsWith('Subscription_') || order.note === 'Autopay_Setup_3DayTrial')) {
      ...
      if (error) {
        ...
        console.log(`Successfully renewed subscription for ${targetMerchantId} (${isTrial ? '3-Day Autopay Trial + 30 days active' : `${months} month(s)`}).`);
      }
    }
  } catch (err) {
