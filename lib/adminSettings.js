  ...
  try {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      ...
      if (!error && user) {
        ...
        if (settings && settings.admin_email && user.email === settings.admin_email) {
          return true;
        }
      }
    }
...
export async function checkAndProcessSubscription(order, apiKey) {
  try {
    ...
    if (isPlatformMerchant && order.note?.startsWith('Subscription_') && order.external_ref) {
      ...
      if (error) {
        ...
        console.log(`Successfully renewed subscription for ${targetMerchantId} by ${months} month(s).`);
      }
    }
  } catch (err) {
